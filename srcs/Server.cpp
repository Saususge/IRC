...
void Server::setNonBlockingOrDie(int fd) {
  if (fcntl(fd, F_SETFL, O_NONBLOCK) == -1) exit_with_error("Fcntl failed");
}

void Server::initSocketOrDie() {
  _serverFd = ::socket(AF_INET, SOCK_STREAM, 0);
  if (_serverFd == -1) exit_with_error("Socket creation failed");
  ...
  if (::listen(_serverFd, 10) == -1) exit_with_error("Listen failed");
...
void Server::run() {
  ...
  while (true) {
    ...
    if (ret == -1) exit_with_error("Poll failed");

    for (size_t i = 0; i < _pollFds.size();) {
      int fd = _pollFds[i].fd;

      if (_pollFds[i].revents & (POLLHUP | POLLERR | POLLNVAL)) {
        if (fd == _serverFd) exit_with_error("Server socket error");
        closeClient(i);
        continue;
      }

      if (fd == _serverFd) {
        if (_pollFds[i].revents & POLLIN) acceptClients();
        ++i;
        continue;
      }

      if (_pollFds[i].revents & POLLIN) handleClientReadable(i);

      // handleClientReadable() may have closed the client (and erased this poll
      // entry).
      if (i >= _pollFds.size() || _pollFds[i].fd != fd) continue;

      if (_pollFds[i].revents & POLLOUT) handleClientWritable(i);

      // If the client got closed, the current index now points to the next fd.
      if (i < _pollFds.size() && _pollFds[i].fd == fd) ++i;
    }
  }
}
...
void Server::updatePollEvents(int fd) {
  ...
  if (it != _outbuf.end() && !it->second.empty()) events |= POLLOUT;
  _pollFds[i].events = events;

  // 4. 성공 로그 (실제로 설정 후)
  std::cout << "[DEBUG] fd=" << fd
            << " events set to: " << (events & POLLIN ? "POLLIN " : "")
            << (events & POLLOUT ? "POLLOUT" : "") << std::endl;
}
